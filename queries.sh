...
echo "$($PSQL "SELECT AVG(winner_goals) AS winner_goals FROM games")"
...
echo "$($PSQL "SELECT MAX(winner_goals) AS number_goals FROM games")"

echo -e "\nNumber of games where the winning team scored more than two goals:"
echo "$($PSQL "SELECT COUNT(*) AS number_games FROM games WHERE winner_goals > 2 ")"
